 ...
 * Now with Thermer App Integration for Android devices
 */

import ThermerIntegration from './thermer-integration.js';
...
export class ThermalPrinter {
  constructor() {
    ...
    // Initialize Thermer integration
    this.thermerIntegration = new ThermerIntegration();
    this.preferThermer = true; // Default to prefer Thermer on Android
  ...
  /**
   * Smart print method that prioritizes Thermer on Android
   */
  async smartPrint(receiptData, options = {}) {
    const { 
      forceThermer = false, 
      forceWebBluetooth = false,
      fallbackToWebBluetooth = false // Default false for mobile-first approach
    } = options;

    console.log('ThermalPrinter: Starting smart print', { receiptData, options });
    
    const thermerStatus = this.thermerIntegration.getStatus();
    ...
    if (forceThermer) {
      return await this.thermerIntegration.print(receiptData);
    }

    // MOBILE (Android): Prioritize Thermer Intent
    if (isAndroid && thermerStatus.thermerSupported) {
      try {
        console.log('Android detected: Using Thermer Intent...');
        const result = await this.thermerIntegration.print(receiptData);
        
        return {
          success: true,
          method: 'thermer',
          message: result.message || 'Receipt dikirim ke Thermer app'
        };
      } catch (error) {
        console.warn('Thermer Intent failed:', error);
        
        if (!fallbackToWebBluetooth) {
          throw new Error(`Thermer Intent gagal: ${error.message}. Pastikan Thermer app sudah terinstall.`);
        }
      }
    }

    // DESKTOP: Use Web Bluetooth only
    if (!isAndroid) {
      try {
        console.log('Desktop detected: Using Web Bluetooth...');
        ...
        throw new Error(`Web Bluetooth gagal: ${error.message}`);
      }
    }

    // Fallback to Web Bluetooth for Android (if enabled)
    if (fallbackToWebBluetooth) {
      try {
        console.log('Using Web Bluetooth fallback...');
        await this.printReceipt(receiptData);
        
        return {
          success: true,
          method: 'web-bluetooth',
          message: 'Receipt printed via Web Bluetooth fallback'
        };
      } catch (error) {
    ...
    // If we reach here, no suitable method available
    throw new Error('Tidak ada metode printing yang tersedia untuk platform ini');
  }
}
