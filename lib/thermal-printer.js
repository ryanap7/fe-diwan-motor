...
export class ThermalPrinter {
  ...
  /**
   * Smart print method that chooses between Thermer and Web Bluetooth
   */
  async smartPrint(receiptData, options = {}) {
    ...
    if (thermerStatus.thermerSupported && this.preferThermer) {
      try {
        console.log('Attempting Thermer print...');
        ...
        console.warn('Thermer print failed, trying Web Bluetooth fallback:', error);
   ...
   */
  setPreference(preferThermer = true) {
    this.preferThermer = preferThermer;
    localStorage.setItem('thermal_printer_prefer_thermer', preferThermer.toString());
  }
   ...
   */
  async getStatus() {
    const webBluetoothSupported = !!navigator.bluetooth;
    const thermerStatus = this.thermerIntegration.getStatus();
    ...
    return {
      ...
      thermer: thermerStatus,
      recommendation: thermerStatus.thermerSupported ? 'thermer' : 'web-bluetooth',
      currentPreference: this.preferThermer ? 'thermer' : 'web-bluetooth'
    };
  }
  ...
  async testAllMethods() {
    ...
    // Test Thermer
    try {
      const thermerResult = await this.thermerIntegration.testConnection();
      results.thermer = {
        success: true,
        message: thermerResult.message
      };
    } catch (error) {
      results.thermer = {
        success: false,
    ...
    try {
      if (this.isConnected) {
        results.webBluetooth = {
          success: true,
          message: 'Web Bluetooth connection active'
        };
      } else {
        results.webBluetooth = {
          success: false,
          error: 'Web Bluetooth not connected'
        };
      }
    } catch (error) {
      results.webBluetooth = {
