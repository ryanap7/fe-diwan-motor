...
export class ThermalPrinter {
  ...
  async printReceipt(receiptData) {
    try {
      ...
      output += safePrint(receiptData.storeName || 'TOKO') + '\n';
      output += this.commands.BOLD_OFF;
      output += safePrint(receiptData.storeAddress || '') + '\n';
      output += 'Tel: ' + safePrint(receiptData.phoneNumber || '') + '\n';
      output += this.commands.LINE_FEED;

      // Receipt info
      output += this.commands.ALIGN_LEFT;
      output += 'Invoice: ' + safePrint(receiptData.invoiceNo || '') + '\n';
      output += 'Tanggal: ' + safePrint(receiptData.date || '') + '\n';
      output += 'Waktu: ' + safePrint(receiptData.time || '') + '\n';
      output += 'Kasir: ' + safePrint(receiptData.customerName || 'Walk-in') + '\n';
      if (receiptData.customerPhone && receiptData.customerPhone !== '-') {
        output += 'HP: ' + safePrint(receiptData.customerPhone) + '\n';
      }
      output += this.commands.LINE_FEED;
