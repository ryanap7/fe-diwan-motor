...
export class RawBTIntegration {
  ...
  async printViaRawBT(receiptData) {
    try {
      ...
      // Clean the receipt text to remove any URL scheme prefixes
      const cleanReceiptText = receiptText.replace(/^\/\/print\?text=/, '');
      
      // Create intent URL for RawBT using proper format
      const intentUrl = `intent://#Intent;action=${this.rawbtConfig.intentAction};type=${this.rawbtConfig.dataType};package=${this.rawbtConfig.packageName};S.android.intent.extra.TEXT=${encodeURIComponent(cleanReceiptText)};end`;
